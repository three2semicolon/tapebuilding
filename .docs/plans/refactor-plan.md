# Refactor Plan for the tapebuilding Repository

**Goal**  
Create a clean, reusable package that can be driven by multiple pipelines (e.g., “pull updates → store list → download missing → refresh playlists”) and eventually by a lightweight local UI. The package should eliminate duplication, centralize configuration, and provide a composable API.
...
├─ organize/
│   ├─ __init__.py
│   ├─ beets_import.py         # unchanged, now imports tapebuilding.importer
...
## 3. Example New API Usage

```bash
# Dry‑run (prints commands, no changes)
tapebuilding sync --only download,import,playlist --dry-run

# Real execution
tapebuilding sync --apply --only download,import,playlist

# Run only the download step (useful for debugging)
tapebuilding sync --only download --apply
```
...
```python
pipeline = Pipeline()
pipeline.add_step('download', lambda ctx: downloader.download(...))
pipeline.add_step('import', lambda ctx: importer.import_drop(...))
pipeline.add_step('playlist', lambda ctx: playlist_builder.build(...))
pipeline.run(args)
```

---

## 4. Immediate Cleanup Wins
